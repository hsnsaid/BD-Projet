...
<html lang="en">
    <head>
        ...
        <script src="../front_script/admin/update.js" defer></script>
    </head>
    <body>
    <div class="body"></div>
        <header class="">
...
include 'db.php';
$id=$_GET['id'];
$_SESSION['old_etudaint_id']=$id;
$sql="SELECT Etudiant_name,niveau,Etudiant_Group,date_of_birth,Place_of_birth,password FROM etudiant WHERE id=$id";
$result=$conn->query($sql);
$array=$result->fetch_all(MYSQLI_ASSOC);
}
?>
      <div class="successfull">
        <h2 class="message">Student has been Upadated</h2>
      </div>
      <h1 class="add_student">Update Student</h1>
      <hr>
      <main>
        <div class="login">
            <form action="" method="post" class="form">
              <div class="user">
                <input type="text" name="name" value="<?php echo($array[0]['Etudiant_name'])  ?>" required  >
              ...
              <div class="user">
                ...
                <option  value="<?php echo($array[0]['niveau'])  ?>"><?php echo($array[0]['niveau']) ?></option>
              ...
              <div class="user">
                <input type="text" name="Etudiant_Group"  value="<?php echo($array[0]['Etudiant_Group'])  ?>" required >
                <label>Group</label>
              </div>
              <div class="user">
                <input type="date" name="date_of_birth" value="<?php echo($array[0]['date_of_birth'])  ?>" required>
                <label class="date">Date of Birth</label>
              </div>
              <div class="user">
                <input type="text" name="Place_of_birth" value="<?php echo($array[0]['Place_of_birth'])  ?>" required >
